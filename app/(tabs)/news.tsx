...
type NewsItem = {
  ...
  title: string;
  summary: string;
  body: string[];          // paragraphs for detail view
  metrics?: Metric[];
  time: string;
  source: string;
...
// ─── Real news from FY2025 audited results ─────────────────────────────────────
...
const NEWS_ITEMS: NewsItem[] = [
  // ── FDH Bank Hero ──────────────────────────────────────────────────────────
  {
    ...
    summary:
      "Net interest income surged 82% and total assets crossed MWK 1.6 trillion as FDH Bank reports its strongest annual performance on record.",
    body: [
  ...
  // ── FDH Bank Mozambique ───────────────────────────────────────────────────
  {
    ...
    summary:
      "FDH completed the acquisition of a 98.87% controlling stake in Ecobank Mozambique SA on 5 September 2025, marking its entry into the Southern African market.",
    body: [
  ...
  // ── NITL Profit ───────────────────────────────────────────────────────────
  {
    ...
    summary:
      "National Investment Trust Ltd recorded a 579% jump in net profit, driven by MWK 201.9 billion in fair value gains as the MSE delivered its strongest year on record.",
    body: [
  ...
  // ── NITL Dividend ─────────────────────────────────────────────────────────
  {
    ...
    summary:
      "The Board approved a second interim dividend of MWK 6.00 per share, with a first interim of MWK 5.00 paid in October 2025, bringing the FY2025 total to MWK 11.00.",
    body: [
  ...
  // ── NICO Holdings Main ────────────────────────────────────────────────────
  {
    ...
    summary:
      "Gross revenue climbed 74% to MWK 919.3 billion as NBS Bank and NICO Life drove record results across the Group's diversified financial services portfolio.",
    body: [
      ...
      "The Group invested MWK 1.3 billion in corporate social responsibility initiatives in 2025, channelling funds into community food relief, education, sports development, arts and culture, and healthcare.",
    ],
    metrics: [
  ...
  // ── NICO Life ─────────────────────────────────────────────────────────────
  {
    ...
    summary:
      "Strong investment returns from fixed-income instruments and listed equities, combined with new business wins, drove NICO Life's standout FY2025 performance.",
    body: [
      ...
      "As part of the broader NICO Group, NICO Life's strong performance contributed significantly to the Group's consolidated Profit After Tax of MWK 323.5 billion, making it the highest-earning segment within the NICO portfolio for FY2025.",
    ],
    metrics: [
  ...
  // ── NICO Dividend ─────────────────────────────────────────────────────────
  {
    ...
    summary:
      "The Board has proposed a final dividend of MWK 8.05 per share, bringing total FY2025 distributions to MWK 40.05 per share — double the MWK 20.00 paid in 2024.",
    body: [
      ...
      "Including the three interim dividends already paid, total distributions in respect of FY2025 amount to MWK 40.05 per share — comprising a first interim of MWK 6.00 (paid 22 September 2025), a second interim of MWK 6.00 (paid 29 December 2025), a third interim of MWK 20.00, and the proposed final dividend of MWK 8.05.",
      ...
      "Shareholders on the register at the close of business on Friday, 10 April 2026 will be eligible for the third interim dividend, payable on 20 April 2026. The register will be closed from 11 to 13 April 2026, with the ex-dividend date of Wednesday, 8 April 2026.",
      "With 1,043,041,000 shares in issue, the total FY2025 dividend payout amounts to approximately MWK 41.774 billion, compared with MWK 20.861 billion paid in FY2024.",
    ],
    metrics: [
...
function CloseIcon() {
  return (
    <Svg width={20} height={20} viewBox="0 0 24 24" fill="none">
      <Line x1="18" y1="6" x2="6" y2="18" stroke={DARK} strokeWidth={2} strokeLinecap="round"/>
      <Line x1="6" y1="6" x2="18" y2="18" stroke={DARK} strokeWidth={2} strokeLinecap="round"/>
    </Svg>
  );
}
...
function DetailModal({ item, onClose }: { item: NewsItem; onClose: () => void }) {
  ...
  return (
    <Modal animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
      <View style={[detail.root, { paddingTop: Platform.OS === "ios" ? insets.top : 0 }]}>
        {/* Close bar */}
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={detail.scroll}>
          ...
          <View style={detail.metaRow}>
            <View style={[detail.catBadge, { backgroundColor: TEAL + "18" }]}>
              <Text style={[detail.catBadgeText, { color: TEAL }]}>{item.category}</Text>
            </View>
            <Text style={detail.metaTime}>{item.time}</Text>
            <Text style={detail.metaDot}>·</Text>
            <Text style={detail.metaSource}>{item.source}</Text>
          </View>
          ...
          {item.metrics && item.metrics.length > 0 && (
            <View style={detail.metricsSection}>
              <Text style={detail.metricsHeading}>Key Metrics</Text>
              <View style={detail.metricsGrid}>
                {item.metrics.map((m, i) => (
                  <View key={i} style={detail.metricCard}>
                    <Text style={detail.metricLabel}>{m.label}</Text>
                    <Text style={[detail.metricValue, m.up !== undefined && { color: m.up ? GREEN : "#EF4770" }]}>
                      {m.value}
                    </Text>
                  </View>
                ))}
          ...
          {/* Body paragraphs */}
          <View style={detail.bodySection}>
            {item.body.map((para, i) => (
              <Text key={i} style={detail.bodyPara}>{para}</Text>
            ))}
          </View>
    ...
    <TouchableOpacity activeOpacity={0.88} style={styles.heroCard} onPress={onPress}>
      ...
      <View style={styles.heroBody}>
        ...
        <View style={styles.metaRow}>
          ...
          <Text style={[styles.metaCategory, { color: TEAL }]}>{item.category}</Text>
          <View style={{ flex: 1 }} />
  ...
  return (
    <View style={[styles.container, { paddingTop: topPad }]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.listContent}>
        {hero && <HeroCard item={hero} onPress={() => setSelected(hero)} />}
        {rest.length > 0 && <View style={styles.sectionDivider} />}
        {rest.map((item, i) => (
          <React.Fragment key={item.id}>
            <ThumbCard item={item} onPress={() => setSelected(item)} />
...
const styles = StyleSheet.create({
  ...
  sectionDivider:{ height: 1, backgroundColor: CARD_BORDER, marginVertical: 20, marginHorizontal: 20 },
  thumbDivider:  { height: 1, backgroundColor: CARD_BORDER, marginVertical: 16, marginHorizontal: 20 },
...
const detail = StyleSheet.create({
  ...
  topBar: { flexDirection: "row", alignItems: "center", justifyContent: "center", paddingTop: 12, paddingHorizontal: 20, paddingBottom: 8, position: "relative" },
  handle: { width: 36, height: 4, borderRadius: 2, backgroundColor: "#E5E7EB" },
  closeBtn: { position: "absolute", right: 20, top: 10, width: 36, height: 36, borderRadius: 18, backgroundColor: "#F3F4F6", alignItems: "center", justifyContent: "center" },
